...
def getAllProcessMetricsForSingleFile(full_path_param, repo_path_param, org_of_file, full_categ_df, window_size):
      process_metrics =  stallings_miner.getStallingsMetrics(full_path_param, repo_path_param, org_of_file, full_categ_df, window_size)
      print "-"*50
...
def getAllProcessMetricForAllFiles(pupp_map_dict_param, datasetFile2Save, org_name, full_categ_df, window_size):
   str2ret=''
   fileCount = 0
   for file_, details_ in pupp_map_dict_param.items():
           ...
           if (file_!= 'WTF') and (os.path.exists(file_)):
              ...
              all_metric_for_this_file = getAllProcessMetricsForSingleFile(file_, repo_, org_name, full_categ_df, window_size)
...
if __name__=='__main__':
  ...
  str_ = getAllProcessMetricForAllFiles(fullPuppMap, datasetFile2Save, org_name, full_categ_df, window_)
  print "-"*100
